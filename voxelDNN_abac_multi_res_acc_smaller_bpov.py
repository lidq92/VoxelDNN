...
def VoxelDNN_encoding(args):
    ...
    with contextlib.closing(arithmetic_coding.BitOutputStream(open(outputfile, "wb"))) as bitout, contextlib.closing(arithmetic_coding.BitOutputStream(open(outputfile + 'test', "wb"))) as bitest:
        heatmap, flags, no_oc_voxels = voxelDNN_encoding_slave(boxes, voxel_DNN, bitout,bitest, flags, bl_par_depth, bbox_max)
...
def voxelDNN_encoding_slave(oc, voxel_DNN, bitout, bitest, flags, par_bl_level, bbox_max):
    static=[]
    ...
    for i in range(oc.shape[0]):
        ...
        op3, flag3 = encode_child_box_test(box, voxel_DNN, test_enc, bitest, curr_level, max_level, bbox_max)
        idx = 0
        _, curr_bits_cnt = encode_child_box_worker(box, voxel_DNN, enc, bitout, flag3, 
                                                   idx, curr_bits_cnt, curr_level, max_level, bbox_max)
        for fl in flag3:
            flags.append(fl)
...
def encode_single_box(box, voxel_DNN, enc, bitstream, bbox_max):
    # encoding block as one using voxel DNN
...
def encode_child_box_test(box, voxel_DNN, test_enc, bitest, curr_level, max_level, bbox_max):
            ...
            for w in range(2):
                ...
                if np.sum(child_box) == 0.:
                    ...
                    if(curr_level == max_level):
                        child_no_bits = encode_single_box(child_box, voxel_DNN, test_enc, bitest, bbox_max)
                        child_flags.append(1)
                        child_no_bits += 2

                    else:
                        #encoding as one
                        op1 = encode_single_box(child_box, voxel_DNN, test_enc, bitest, bbox_max)
                        op1 += 2

                        #encoding using 8 sub child blocks
                        op2, rec_child_flag = encode_child_box_test(child_box, voxel_DNN, test_enc, 
                                                                    bitest, curr_level + 1, max_level, bbox_max)
                        if op2 > op1:
                            child_no_bits = op1
                            child_flags.append(1)
                        else:
    ...
    no_bits1 = encode_single_box(box, voxel_DNN, test_enc, bitest, bbox_max) + 2
    flag1 = [1]
    if (no_bits > no_bits1):
...
def encode_child_box_worker(box, voxel_DNN, enc, bitout, flag, idx, bit_cnt, curr_level, max_level, bbox_max):
    # box --> flag 0 if child box is empty;
            ...
            for h in range(2):
                for w in range(2):
                    ...
                    if ocv == 0.:
                        ...
                        if(curr_level == max_level):
                            bit_cnt[curr_level].append([encode_single_box(child_box, voxel_DNN, enc,bitout, bbox_max), ocv])
                            ...
                            #idx+=1
                                idx, bit_cnt = encode_child_box_worker(child_box, voxel_DNN, enc, bitout, flag, 
                                                                       idx, bit_cnt, curr_level + 1, max_level, bbox_max)
    elif flag[idx] == 1:
        ocv = np.sum(box)
        bit_cnt[curr_level - 1].append([ocv, encode_single_box(box, voxel_DNN, enc, bitout, bbox_max)])
        idx += 1
    return idx, bit_cnt
...
if __name__ == "__main__":
    ...
    parser.add_argument("-departition", '--departition_level', type=int, default=6,
                        help='depth of input octree to pass for encoder')
    ...
    VoxelDNN_encoding([args.octreedepth, args.departition_level, args.plypath, args.modelpath, args.partitioningdepth])
